...
const navLinks = [
  ...
  { name: "About Us", href: "/about" },
  {
    name: "Our Team",
    dropdown: [
      { name: "Board-Member", href: "/chairman" },
      // { name: "Resource Persons", href: "#" },
      { name: "Change-makers", href: "/changeMaker" },
    ],
  },
  {
    name: "Get Involved",
    dropdown: [
      ...
      {
        name: "Open a Chapter",
        href: "https://forms.gle/5GYLHndDiLfrc5br6",
        external: true,
      },
      {
        name: "Register as a Mentor",
        href: "https://forms.gle/JjMmTJdTxCkhxzsNA",
        external: true,
      },
    ],
  },
...
const gradient =
  "bg-gradient-to-r from-sky-700 via-emerald-600 to-indigo-700";
...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50">
      <nav
        className={`w-full ${gradient} shadow-lg px-4 md:px-10 py-2 transition-all duration-300`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            <Image
              ...
              priority
            />
            <span className="hidden sm:block text-xl font-extrabold bg-gradient-to-r from-white via-teal-200 to-yellow-200 bg-clip-text text-transparent tracking-wide drop-shadow">
              BIIN
            </span>
          </Link>
            ...
            {navLinks.map((link, idx) =>
              link.dropdown ? (
                <li key={link.name} className="relative group">
                  <button
                    className="flex items-center gap-1 px-3 py-2 rounded-lg transition text-white hover:bg-white/10 hover:text-teal-300 focus:outline-none"
                  >
                  ...
                  <ul className="absolute left-0 top-full mt-2 min-w-[180px] bg-white shadow-xl rounded-xl py-2 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 translate-y-2 pointer-events-none group-hover:pointer-events-auto transition-all duration-200 z-40">
                ...
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="px-3 py-2 rounded-lg text-sm text-white hover:bg-white/10 hover:text-teal-300 transition"
                  >
                    {link.name}
                  </Link>
                </li>
          ...
          >
            {!isOpen ? (
              <BsMenuButtonWideFill size={28} color="#fff" />
            ) : (
              <BsFillBackspaceFill size={28} color="#fff" />
            )}
          </button>
        </div>
      </nav>
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b">
            <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2">
              <Image
                ...
                priority
              />
              <span className="text-lg font-bold bg-gradient-to-r from-sky-700 via-emerald-600 to-indigo-700 bg-clip-text text-transparent">
                BIIN
              </span>
            </Link>
            <button
